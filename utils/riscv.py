...
class Riscv:
    ...
    CallerSaved = [T0, T1, T2, T3, T4, T5, T6, A1, A2, A3, A4, A5, A6, A7] # 删除A0试试？
    ...
    class Call(TACInstr):
        def __init__(self, output: Temp, func_label: Label) -> None:
            super().__init__(InstrKind.SEQ, [output], [], func_label)
            self.func_label = func_label
        
        def __str__(self) -> str:
            return "call " + str(self.func_label.func) 
        
    class LoadSymbol(TACInstr):
        def __init__(self, output: Temp, symbol: str) -> None:
            super().__init__(InstrKind.SEQ, [output], [], symbol)
            self.symbol = symbol
    ...
    class LoadGlobal(TACInstr):
        def __init__(self, output: Temp, src: Temp, offset:int) -> None:
            super().__init__(InstrKind.SEQ, [output], [src], None)
            self.offset = offset
    ...
    class StoreGlobal(TACInstr):
        def __init__(self, input: Temp, src: Temp, offset:int) -> None:
            super().__init__(InstrKind.SEQ, [], [input,src], None)
            self.offset = offset
    ...
    class Alloc(TACInstr):
        def __init__(self, output:Temp, offset:int) -> None:
            super().__init__(InstrKind.SEQ, [output], [], None)
            self.offset = offset
