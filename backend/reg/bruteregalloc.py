...
class BruteRegAlloc(RegAlloc):
    ...
    def allocForLoc(self, loc: Loc, subEmitter: SubroutineEmitter):
        ...
        if isinstance(instr,Riscv.Param):
            # 将所有参数都放到函数栈上
            offset = 4 * instr.index
            reg  = self.allocRegFor(instr.srcs[0],True,loc.liveIn,subEmitter)
            subEmitter.emitNative(Riscv.NativeStoreWord(reg, Riscv.SP, offset))
        
        elif isinstance(instr, Riscv.Call):
            temp_list = []
            # 保存 caller-saved register
            for i in range(len(Riscv.CallerSaved)):
                reg = Riscv.CallerSaved[i]
                if reg.isUsed():
                    temp_list.append(reg.temp)
            ...
            # 恢复 caller-saved register
            for i in range(len(temp_list)):
                if temp_list[i] is not None:
                    subEmitter.emitLoadFromStack(Riscv.CallerSaved[i], temp_list[i])

            # 返回结果
            subEmitter.emitNative(Riscv.Move(dstRegs[0], Riscv.A0).toNative(dstRegs, [Riscv.A0]))
